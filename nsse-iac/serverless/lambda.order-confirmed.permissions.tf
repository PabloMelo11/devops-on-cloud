resource "aws_iam_role" "lambda_confirmed_lambda_role" {
  name               = var.lambda_order_confirmed.role_name
  assume_role_policy = data.aws_iam_policy_document.lambda_assume_role.json
}

resource "aws_iam_policy" "order_confirmed_lamdda_policy" {
...
resource "aws_iam_role_policy_attachment" "order_confirmed_lambda_vpc" {
  ...
  role       = aws_iam_role.lambda_confirmed_lambda_role.name
}

resource "aws_iam_role_policy_attachment" "order_confirmed_lambda_custom" {
  policy_arn = aws_iam_policy.order_confirmed_lamdda_policy.arn
  role       = aws_iam_role.lambda_confirmed_lambda_role.name
}
