...
variable "assume_role" {
  ...
  default = {
    ...
    session_name = "nsse-terraform-server"
  }
}
...
variable "s3_application_bucket_name" {
  type    = string
  default = "nsse-production-application-bucket"
}
...
variable "lambda_report_job" {
  ...
  default = {
    timeout       = 30
    package_type  = "zip"
    source_dir    = "lambdas/report_job/build"
    output_path   = "lambdas/report_job/outputs/package.zip"
    filename      = "lambdas/report_job/outputs/package.zip"
    function_name = "reportJobLambdaFunction"
    handler       = "index.handler"
    runtime       = "nodejs18.x"
    role_name     = "nsse-production-report_job-lambda-role"
    policy_name   = "nsse-production-report_job-lambda-policy"
  }
}
...
variable "document_db_cluster" {
  type = object({
    ...
    default = {
      ...
      instance = {
        identifier = "nsse-document-db-cluster-single-instance"
        class      = "db.t3.medium"
      }
    }
  })
