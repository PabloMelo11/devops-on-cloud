...
variable "ec2_resources" {
  ...
  default = {
    key_pair_name                = "nsee-production-key-pair"
...
variable "debian_patch_baseline" {
  ...
  default = {
    ...
    description                          = "Debian Patch Baseline for Debian Production Servers"
...
variable "debian_production_ssm_association" {
  type = object({
    name                = string
...
variable "ssm_patching_logs_bucket" {
  ...
  default = {
    name          = "nsse-production-ssm-patching-logs"
    force_destroy = true
  }
}
